...
int bind_server_udp1(int server_s)
{
	...
	local_addr.sin_addr.s_addr = inet_addr("192.168.124.44");
...
{  
    ...
    soap_init1(&add_soap,SOAP_IO_UDP);  
    ...
    if (argc < 2) {  
        ...
        m = soap_bind(&add_soap, "192.168.124.44", atoi(argv[1]), 100);  
        if (m < 0) {  
...
{
	fprintf(stderr,"SOAP_ENV__Fault\n");
	return 0;
}
