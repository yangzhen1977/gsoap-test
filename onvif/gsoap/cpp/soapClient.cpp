...
SOAP_SOURCE_STAMP("@(#) soapClient.cpp ver 2.8.28 2017-11-27 13:14:20 GMT")
...
{	struct SOAP_ENV__Fault_yz soap_tmp_SOAP_ENV__Fault_yz;
	soap_tmp_SOAP_ENV__Fault_yz.faultcode = faultcode;
	soap_tmp_SOAP_ENV__Fault_yz.faultstring = faultstring;
	soap_tmp_SOAP_ENV__Fault_yz.faultactor = faultactor;
	soap_tmp_SOAP_ENV__Fault_yz.detail = detail;
	soap_tmp_SOAP_ENV__Fault_yz.SOAP_ENV__Code = SOAP_ENV__Code;
	soap_tmp_SOAP_ENV__Fault_yz.SOAP_ENV__Reason = SOAP_ENV__Reason;
	soap_tmp_SOAP_ENV__Fault_yz.SOAP_ENV__Node = SOAP_ENV__Node;
	soap_tmp_SOAP_ENV__Fault_yz.SOAP_ENV__Role = SOAP_ENV__Role;
	soap_tmp_SOAP_ENV__Fault_yz.SOAP_ENV__Detail = SOAP_ENV__Detail;
	soap_begin(soap);
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	soap_serialize_SOAP_ENV__Fault_yz(soap, &soap_tmp_SOAP_ENV__Fault_yz);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_SOAP_ENV__Fault_yz(soap, &soap_tmp_SOAP_ENV__Fault_yz, "SOAP-ENV:Fault-yz", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
...
{
	soap_default_SOAP_ENV__Fault_yz(soap, _param_8);
	soap_begin(soap);
	if (soap_begin_recv(soap)
	 || soap_envelope_begin_in(soap)
	 || soap_recv_header(soap)
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	soap_get_SOAP_ENV__Fault_yz(soap, _param_8, "SOAP-ENV:Fault-yz", NULL);
	if (soap->error == SOAP_TAG_MISMATCH && soap->level == 2)
		soap->error = SOAP_OK;
	if (soap->error
	 || soap_body_end_in(soap)
